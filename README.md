...
Destinations are stored universally in Vercel Edge Config through `/api/destinations` under the key `apartment_hunt_destinations`. Apartments are stored through `/api/apartments` under `apartment_hunt_apartments`. Attach Edge Config to the project so Vercel injects `EDGE_CONFIG`. Saving from the app also needs a Vercel API token with access to update that Edge Config.
